...
export default function FeedbackPanel({ feedback, xpEarned }: FeedbackPanelProps) {
  ...
  const criteriaLabels: Record<string, string> = {
    thesis_clarity: t(T.feedback.criteria.thesis_clarity),
    evidence_quality: t(T.feedback.criteria.evidence_quality),
    language_analysis: t(T.feedback.criteria.language_analysis),
    coherence: t(T.feedback.criteria.coherence),
    contextual_awareness: t(T.feedback.criteria.contextual_awareness),
  }
    ...
    <div className="space-y-4">
      {xpEarned > 0 && (
        <div className="bg-gradient-to-r from-indigo-500 to-purple-600 rounded-2xl p-4 text-white text-center shadow-md">
          ...
          <p className="text-sm opacity-80 mt-0.5">{t(T.feedback.essayReward)}</p>
        </div>
      )}

      <div className="bg-white rounded-2xl border border-gray-200 p-5">
        <div className="flex items-center justify-between mb-3">
          <h3 className="font-bold text-gray-800">{t(T.feedback.overallScore)}</h3>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-5 space-y-4">
        <h3 className="font-bold text-gray-800">{t(T.feedback.criteriaFeedback)}</h3>
        {Object.entries(feedback.scores).map(([key, val]) => (
          <div key={key} className="border-b border-gray-100 last:border-0 pb-4 last:pb-0">
            <div className="flex items-center justify-between mb-1.5">
              <span className="text-sm font-medium text-gray-700">{criteriaLabels[key]}</span>
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-2xl p-5">
        <h3 className="font-bold text-amber-800 mb-2 text-sm">{t(T.feedback.modelSentence)}</h3>
        <p className="text-sm text-amber-700 italic leading-relaxed">"{feedback.model_sentence}"</p>
      </div>

      <div className="bg-indigo-50 border border-indigo-100 rounded-2xl p-5">
        <h3 className="font-bold text-indigo-800 mb-2 text-sm">{t(T.feedback.overallComment)}</h3>
